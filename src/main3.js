...
class LINE extends Command {
    ...
    async textMessage(messages) {
        ...
        this.command('bot3', ['Bot3 hadir']);
        this.command('kamu siapa', this.getProfile.bind(this));
        this.command('bot3 Status', `Your Status: ${JSON.stringify(this.stateStatus)}`);
        this.command(`3left ${payload}`, this.leftGroupByName.bind(this));
        this.command('bot3 speed', this.getSpeed.bind(this));
        ...
        this.command(`3dadah ${payload}`,this.kickAll.bind(this));
        this.command(`3cancel ${payload}`, this.cancelMember.bind(this));
        this.command(`bot3 set`,this.setReader.bind(this));
        this.command(`bot3 cek`,this.rechecks.bind(this));
        this.command(`bot3 clearall`,this.clearall.bind(this));
        this.command('bot3 myid',`Your ID: ${messages.from}`)
        this.command(`Modyarr ${payload}`,this.checkIP.bind(this))
        this.command(`bot3_ig ${payload}`,this.checkIG.bind(this))
        this.command(`bot3_qr ${payload}`,this.qrOpenClose.bind(this))
        this.command(`bot3_joinqr ${payload}`,this.joinQr.bind(this));
        this.command(`bot3_spam ${payload}`,this.spamGroup.bind(this));
        this.command(`creator`,this.creator.bind(this));

        this.command(`bot3_pap ${payload}`,this.searchLocalImage.bind(this));
        this.command(`bot3_upload ${payload}`,this.prepareUpload.bind(this));
        this.command(`bot3_vn ${payload}`,this.vn.bind(this));
